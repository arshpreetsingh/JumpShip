...
func MakeRecommendPricePlansWeeklyHandler(
	s Service,
	logger *logrus.Entry,
) http.Handler {
	opts := []kithttp.ServerOption{
	...
	endpointHandler := kithttp.NewServer(
		mw(makeRecommendPricePlansWeeklyEndpoint(s)),
		decodeSmartMeterIdFromRequest,
		mhttp.EncodeResponse,
		opts...,
	)
...
// same here need to have function to check/get ID, IF ID Does not Exist,
// Need to return it back!!
// Need to know about this Context part?
