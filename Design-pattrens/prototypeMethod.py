...
'''
abc module in Python provides ABCMeta and abstract.

ABCMeta is meta-Class so we can use inheritance here. @abstractmethod
with required decorator can be used to make an abstract-method inside the class.

So that abstract-method can be Updated as per new inheritance calls.

Base methods will remain same but abstract methods can be updated!
'''
...
class Courses_At_GFG_Cache:
    ...
    def load():
        ...
        # If you use with Courses_At_GFG(metaclass=ABCMeta)
        # that means you can't create Instance of that class.
        # TestABC will throw an error!!
        #testABC = Courses_At_GFG()
        sde = SDE()
        sde.set_id("1")
...
if __name__ == '__main__':
    ...
    print(sde.get_type())
    print(isinstance(sde,Courses_At_GFG_Cache))
